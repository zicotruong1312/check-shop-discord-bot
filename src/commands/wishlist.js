...
async function fetchSkins() {
    try {
        ...
        cachedSkins = response.data.data
            ...
            .map(s => ({
                name: s.displayName,
                value: s.levels[0].uuid
            }));
    } catch (e) {
        console.error('Không load được list súng:', e.message);
    }
}
fetchSkins();
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'add') {
            ...
            if (!skinData) return interaction.reply({ content: '❌ Súng không hợp lệ!', ...EPHEMERAL });

            try {
                await Wishlist.create({ discordId: interaction.user.id, skinId: skinData.value, skinName: skinData.name });
                await interaction.reply({ content: `✅ Đã thêm **${skinData.name}** vào Wishlist. Tớ sẽ thông báo khi nó xuất hiện!`, ...EPHEMERAL });
            } catch (err) {
                if (err.code === 11000) {
                    return interaction.reply({ content: '⚠️ Súng này đã có trong danh sách của bạn rồi.', ...EPHEMERAL });
                }
                console.error(err);
                await interaction.reply({ content: '❌ Lỗi Database.', ...EPHEMERAL });
            }

        } else if (subcommand === 'list') {
            const list = await Wishlist.find({ discordId: interaction.user.id });
            if (list.length === 0) return interaction.reply({ content: '⭐ Wishlist của bạn đang trống.', ...EPHEMERAL });
            ...
            if (result) {
                await interaction.reply({ content: `🗑️ Đã xóa **${result.skinName}** khỏi Wishlist.`, ...EPHEMERAL });
            } else {
                await interaction.reply({ content: '❌ Súng không có trong Wishlist của bạn.', ...EPHEMERAL });
            }
        }
    },
};

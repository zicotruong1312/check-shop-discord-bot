...
const entitlementsToken = 'WILL_FETCH_FRESH';
...
async function run() {
    ...
    // Try all regions with full debug
    ...
    for (const region of regions) {
        ...
        try {
            ...
            console.log(`[${region}] ✅ Status ${res.status} — skins: ${JSON.stringify(res.data.SkinsPanelLayout?.SingleItemOffers)}`);
        } catch(e) {
            const status = e.response ? e.response.status : 'NO_RESPONSE';
            const body = e.response ? JSON.stringify(e.response.data) : e.message;
            console.log(`[${region}] ❌ ${status} — ${body}`);
        }
    }
}

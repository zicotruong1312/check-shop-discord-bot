...
async function test() {
    try {
        console.log("Fetching storefront again...");
        ...
        console.log("Storefront OK:", response.data.SkinsPanelLayout ? "HAS SKINS" : "NO SKINS");
    } catch (e) {
        console.log("Storefront Error:", e.response ? e.response.status : e.message);
        if(e.response) {

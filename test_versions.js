...
// Version strings to try
const VERSIONS = [
    'release-12.06-shipping-19-4440219',  // riotClientVersion from valorant-api.com
    'release-09.00-shipping-1-2679261',   // old hardcoded
    'release-12.06-shipping-19-111.0.0.3261.5663',
];
...
async function run() {
    ...
    for (const clientVersion of VERSIONS) {
        ...
        try {
            ...
            console.log(`✅ [v: ${clientVersion}] Status ${res.status}`);
            ...
            const d = e.response ? JSON.stringify(e.response.data).substring(0, 80) : e.message;
            console.log(`❌ [v: ${clientVersion}] ${s}: ${d}`);
        }
    }
}
